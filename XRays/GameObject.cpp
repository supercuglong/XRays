...
float GameObject::GetPathingRadius() const { return unitInfo->pathRadius; }

float GameObject::GetGameplayRadius() const { return unitInfo->gameplayRadius; }
...
float GameObject::GetHpBarHeight() const { return unitInfo->healthBarHeight; }
...
void GameObject::InitLuaState(lua_State *L_) { lua_pointer_ = L_; }

void GameObject::LoadFromMem(DWORD base, HANDLE hProcess, bool deepLoad) {
  address = base;
  Mem::Read(hProcess, base, buff, sizeBuff);

  std::chrono::duration<float, std::milli> timeDuration =
      std::chrono::high_resolution_clock::now() - timeSinceLastPreviousPosition;
  if (timeDuration.count() > 25) {
    previousPosition = position.clone();
    timeSinceLastPreviousPosition = std::chrono::high_resolution_clock::now();
  }
  ...
  */
  ...
  if (deepLoad) {
    char nameBuff[50];
    Mem::Read(hProcess, Mem::ReadDWORDFromBuffer(buff, Offsets::ObjName),
              nameBuff, 50);
...
void GameObject::LoadChampionFromMem(DWORD base, HANDLE hProcess,
                                     bool deepLoad) {
  memcpy(&spellSlotPointerBuffer, &buff[Offsets::ObjSpellBook],
         sizeof(DWORD) * 6);
  ...
  for (int i = 0; i < 7; ++i) {
    ...
    memcpy(&itemPtr, itemListBuffer + i * 0x10 + Offsets::ItemListItem,
           sizeof(DWORD));
    if (itemPtr == 0)
      continue;
...
void GameObject::LoadBuffFromMem(DWORD base, HANDLE hProcess, bool deepLoad) {
  DWORD buffArrayBgn =
      Mem::ReadDWORD(hProcess, address + Offsets::ObjBuffManager +
                                   Offsets::BuffManagerEntriesArray);
  DWORD buffArrayEnd =
      Mem::ReadDWORD(hProcess, address + Offsets::ObjBuffManager + 0x14);
  int i = 0;
  buffVector.clear();
  buffVector.reserve(i);
  for (DWORD pBuffPtr = buffArrayBgn; pBuffPtr != buffArrayEnd;
       pBuffPtr += 0x8) {
    i++;
    ...
    memcpy(&buffStartTime, &buff[Offsets::BuffEntryBuffStartTime],
           sizeof(float));
    ...
    if (buffCountAlt > 0)
      isAlive = true;
    buffVector.push_back(BuffInstance(buffnamebuffer, isAlive, buffCount,
                                      buffCountAlt, bufftype, buffStartTime,
                                      buffEndTime));
  }
}

float GameObject::GetBasicAttackDamage() { return baseAttack + bonusAttack; }
...
bool GameObject::IsRanged() { return GetBaseAttackRange() >= 300.f; }

luabridge::LuaRef GameObject::ItemsToList() {
  luabridge::LuaRef ret = luabridge::newTable(lua_pointer_);
  for (int i = 0; i < 7; ++i)
    if (!itemSlots[i].isEmpty)
      ret.append(itemSlots[i]);

  return ret;
}
luabridge::LuaRef GameObject::BuffsToList() {
  luabridge::LuaRef ret = luabridge::newTable(lua_pointer_);
  for (auto &ele : buffVector)
    ret.append(ele);
...
// Missile stuff
void GameObject::LoadMissileFromMem(DWORD base, HANDLE hProcess,
                                    bool deepLoad) {
  if (!deepLoad)
    return;

  DWORD spellInfoPtr =
      Mem::ReadDWORDFromBuffer(buff, Offsets::MissileSpellInfo);
  if (spellInfoPtr == 0)
    return;

  DWORD spellDataPtr =
      Mem::ReadDWORD(hProcess, spellInfoPtr + Offsets::SpellInfoSpellData);
  if (spellDataPtr == 0)
    return;
  ...
  Mem::Read(
      hProcess,
      Mem::ReadDWORD(hProcess, spellDataPtr + Offsets::SpellDataMissileName),
      nameBuff, 50);
  if (Character::ContainsOnlyASCII(nameBuff, 50))
    name = Character::ToLower(std::string(nameBuff));
  else
  ...
  // Some spells require their end position to be projected using the range of
  // the spell
  if (spellInfo != GameData::UnknownSpell &&
      HasSpellFlags(ProjectedDestination)) {
...
float GameObject::GetSpeed() const { return spellInfo->speed; }

float GameObject::GetCastRange() const { return spellInfo->castRange; }

float GameObject::GetWidth() const { return spellInfo->width; }

float GameObject::GetCastRadius() const { return spellInfo->castRadius; }

float GameObject::GetDelay() const { return spellInfo->delay; }

float GameObject::GetHeight() const { return spellInfo->height; }

float GameObject::GetTravelTime() const { return spellInfo->travelTime; }

std::string GameObject::GetIcon() const { return spellInfo->icon; }
