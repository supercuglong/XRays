...
public:
  ...
  Vector2 WorldToMinimap(const Vector3 &pos, const Vector2 &wPos,
  ...
  /// Draws a circle at the given coordinate. Coordinates and radius must be in
  /// world space
  void DrawCircleAt(ImDrawList *canvas, const Vector3 &worldPos, float radius,
                    bool filled, int numPoints, ImColor color,
...
private:
  void MultiplyMatrices(float *out, float *a, int row1, int col1, float *b,
                        int row2, int col2);
};