...
bool Input::WasKeyPressed(HKey key) {
  static std::chrono::high_resolution_clock::time_point nowTime;
  static std::chrono::high_resolution_clock::time_point lastTimePressed[300] = {
      std::chrono::high_resolution_clock::now()};
  static bool pressed[300] = {0};

  static std::chrono::duration<float, std::milli> timeDiff;
  ...
  if (keyDown) {
    lastTimePressed[virtualKey] = std::chrono::high_resolution_clock::now();
...
void Input::Move(int x, int y) {
  ...
  mouse_event(MOUSEEVENTF_MOVE | MOUSEEVENTF_VIRTUALDESK | MOUSEEVENTF_ABSOLUTE,
              nx, ny, 0, 0);
  ...
  if (mouse_pos.x != nx || mouse_pos.y != ny)
    mouse_event(MOUSEEVENTF_MOVE | MOUSEEVENTF_VIRTUALDESK |
...
void Input::MouseRightDown() {
  ...
  input.mi.dwFlags =
      MOUSEEVENTF_RIGHTDOWN | MOUSEEVENTF_VIRTUALDESK | MOUSEEVENTF_ABSOLUTE;
  SendInput(1, &input, sizeof(INPUT));
...
void Input::MouseRightUp() {
  ...
  input.type = INPUT_MOUSE;
  input.mi.dwFlags =
      MOUSEEVENTF_RIGHTUP | MOUSEEVENTF_VIRTUALDESK | MOUSEEVENTF_ABSOLUTE;
  SendInput(1, &input, sizeof(INPUT));
  ZeroMemory(&input, sizeof(INPUT));
}
