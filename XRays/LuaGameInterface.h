...
/// Interface used by LUA scripts for game related stuff
...
public:
  ...
  void InitLuaState(lua_State *g_lua_env) {

    champs = luabridge::newTable(g_lua_env);
    minions = luabridge::newTable(g_lua_env);
    turrets = luabridge::newTable(g_lua_env);
    jungle = luabridge::newTable(g_lua_env);
    missiles = luabridge::newTable(g_lua_env);
    others = luabridge::newTable(g_lua_env);

    for (auto &ele : snapshot.champions)
      champs.append(ele.get());

    for (auto &ele : snapshot.minions)
      minions.append(ele.get());

    for (auto &ele : snapshot.turrets)
      turrets.append(ele.get());

    for (auto &ele : snapshot.jungle)
      jungle.append(ele.get());

    for (auto &ele : snapshot.missiles)
      missiles.append(ele.get());

    for (auto &ele : snapshot.others)
      others.append(ele.get());
  }

public:
  LuaGameInterface(MemSnapshot &rhs)
      : snapshot(rhs), champs(NULL), minions(NULL), turrets(NULL), jungle(NULL),
        missiles(NULL), others(NULL) {}

  // Exposed Fields
  luabridge::LuaRef champs, minions, turrets, jungle, missiles, others;
  ...
  GameObject *GetLocalChampion() {
    if (localChampion == nullptr)
      return nullptr;
    return localChampion;
  };

  MapObject *GetMap() { return map; }
  ...
  bool IsScreenPointOnScreen(const Vector2 &point) {
    return ms->renderer->IsScreenPointOnScreen(point, 0.f, 0.f);
  }
  ...
  bool IsWorldPointOnScreen(const Vector3 &point) {
    return ms->renderer->IsWorldPointOnScreen(point, 0.f, 0.f);
  }
  ...
  void DrawText(const Vector2 &pos, const char *text, const ImVec4 &color) {
  ...
  void DrawImageRounded(const char *img, const Vector2 &start,
    ...
    overlay->AddImageRounded(it->second->resourceView, (ImVec2 &)start,
                             (ImVec2 &)end, zero, one, ImColor(color),
                             rounding);
  }
  ...
  bool PointOnLineSegment(const Vector2 &pt1, const Vector2 &pt2,
                          const Vector2 &pt, double epsilon = 0.001) {
  ...
  bool IsPointOnLineSegment(const Vector2 &point, const Vector2 &start,
                            const Vector2 &end) {
  ...
  void PressKey(int key) { return Input::PressKey((HKey)key); }

  bool WasKeyPressed(int key) { return Input::WasKeyPressed((HKey)key); }

  void PressLeftClick() { return Input::PressLeftClick(); }

  void PressRightClick() { return Input::PressRightClick(); }

  void MouseRightDown() { return Input::MouseRightDown(); }

  void MouseRightUp() { return Input::MouseRightUp(); }

  void MouseLeftDown() { return Input::MouseLeftDown(); }

  void MouseLeftUp() { return Input::MouseLeftUp(); }
  ...
  GameObject *GetBestTarget(const UnitTag &unittype, float range) {
    ...
    for (auto &champ : (unittype == Unit_Champion           ? ms->champions
                        : unittype == Unit_Minion_Lane      ? ms->minions
                        : unittype == Unit_Monster          ? ms->jungle
                        : unittype == Unit_Structure_Turret ? ms->turrets
                                                            : ms->champions)) {
      if (champ->HasUnitTags(Unit_Champion_Clone))
        continue;
      if (champ->isAlive && champ->isVisible && champ->isTargetable &&
          champ->IsEnemyTo(*localChampion) && lastHealth >= champ->health &&
          Distance(champ.get(), localChampion) <=
              range + (champ->GetGameplayRadius() / 2)) {
  ...
  static LuaGameInterface ConstructFromMemSnapshot(MemSnapshot &snapshot) {
    LuaGameInterface gs(snapshot);
