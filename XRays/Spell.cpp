...
const HKey Spell::spellSlotKey[6] = {HKey::Q, HKey::W, HKey::E,
                                     HKey::R, HKey::D, HKey::F};
...
const char *Spell::GetTypeStr() { return spellTypeName[(int)slot]; }
...
void Spell::LoadFromMem(DWORD base, HANDLE hProcess, bool deepLoad) {
  ...
  DWORD spellNamePtr =
      Mem::ReadDWORD(hProcess, spellDataPtr + Offsets::SpellDataSpellName);
  DWORD spellMissileNamePtr =
...
float Spell::GetSpeed() const { return info->speed; }

float Spell::GetCastRange() const { return info->castRange; }

float Spell::GetWidth() const { return info->width; }

float Spell::GetCastRadius() const { return info->castRadius; }

float Spell::GetDelay() const { return info->delay; }

float Spell::GetHeight() const { return info->height; }

float Spell::GetTravelTime() const { return info->travelTime; }

std::string Spell::GetIcon() const { return info->icon; }
