...
bool MemoryReader::IsLeagueWindowActive() {
...
void MemoryReader::HookToProcess() {
  ...
  if (0 == IsWow64Process(hProcess, &is64Bit)) {
    throw WinApiException(
        "Failed to identify if process has 32 or 64 bit architecture");
  }
  ...
  if (EnumProcessModules(hProcess, hMods, sizeof(hMods), &cbNeeded)) {
    ...
    std::cout << "moduleAddress => " << std::hex << moduleBaseAddr << std::endl;
  } else {
...
void MemoryReader::ReadRenderer(MemSnapshot &ms) {
  DWORD rendererAddr =
      Mem::ReadDWORD(hProcess, moduleBaseAddr + Offsets::Renderer);
  ms.renderer->LoadFromMem(rendererAddr, moduleBaseAddr, hProcess);
}

void MemoryReader::FindHoveredObject(MemSnapshot &ms) {
  int addrObj =
      Mem::ReadDWORD(hProcess, moduleBaseAddr + Offsets::UnderMouseObject);
...
/// This method reads the game objects from memory. It reads the
/// tree structure of a std::map<int, GameObject*>
/// in this std::map reside Champions, Minions, Turrets, Missiles, Jungle mobs
/// etc. Basically non static objects.
void MemoryReader::ReadObjects(MemSnapshot &ms) {
  ...
  int objectManager =
      Mem::ReadDWORD(hProcess, moduleBaseAddr + Offsets::ObjectManager);
  ...
  while (reads < maxObjects && nodesToVisit.size() > 0) {
    ...
    // Network ids of the objects we are interested in start from 0x40000000. We
    // do this check for performance reasons.
  ...
  for (int i = 0; i < nrObj; ++i) {
    int netId;
    Mem::Read(hProcess, pointerArray[i] + Offsets::ObjNetworkID, &netId,
              sizeof(int));
    ...
    if (it == ms.objectMap.end()) {
      ...
      // If the object changed its id for whatever the fuck reason then we
    ...
    if (obj->networkId != 0) {
      ...
      if (blacklistedObjectNames.find(obj->name) !=
          blacklistedObjectNames.end())
...
void MemoryReader::ReadMinimap(MemSnapshot &snapshot) {
  int minimapObj =
      Mem::ReadDWORD(hProcess, moduleBaseAddr + Offsets::MinimapObject);
  int minimapHud =
      Mem::ReadDWORD(hProcess, minimapObj + Offsets::MinimapObjectHud);
  ...
  memcpy(&snapshot.minimapSize, buff + Offsets::MinimapHudSize,
         sizeof(Vector2));
}

void MemoryReader::FindPlayerChampion(MemSnapshot &snapshot) {
  int netId = 0;
  Mem::Read(hProcess,
            Mem::ReadDWORD(hProcess, moduleBaseAddr + Offsets::LocalPlayer) +
  ...
  if (it != snapshot.objectMap.end())
    ...
    snapshot.player =
        (snapshot.champions.size() > 0 ? snapshot.champions[0] : nullptr);
}

void MemoryReader::ClearMissingObjects(MemSnapshot &ms) {
...
void MemoryReader::MakeSnapshot(MemSnapshot &ms) {
  Mem::Read(hProcess, moduleBaseAddr + Offsets::GameTime, &ms.gameTime,
            sizeof(float));

  // Checking chat
  DWORD chatInstance = Mem::ReadDWORD(hProcess, moduleBaseAddr + Offsets::Chat);
  Mem::Read(hProcess, chatInstance + Offsets::ChatIsOpen, &ms.isChatOpen,
            sizeof(bool));

  // Danger, ban risk!
  /*DWORD zoomInstance = Mem::ReadDWORD(hProcess, moduleBaseAddr +
  Offsets::ZoomClass); float zoomAmount; Mem::Read(hProcess, zoomInstance +
  Offsets::ZoomClass, &zoomAmount, sizeof(float)); float val = 800.0;
  ...
  if (ms.gameTime > 2) {
    ...
    ms.map = std::shared_ptr<MapObject>(MapObject::Get(
        ms.turrets.size() > 10 ? SUMMONERS_RIFT : HOWLING_ABYSS));
  }
}
