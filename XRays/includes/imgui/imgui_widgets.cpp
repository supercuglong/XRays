...
bool ImGui::DragScalar(const char *label, ImGuiDataType data_type, void *p_data,
  ...
  if (!temp_input_is_active) {
    const bool focus_requested =
        temp_input_allowed &&
        (g.LastItemData.StatusFlags & ImGuiItemStatusFlags_Focused) != 0;
...
bool ImGui::InputTextEx(const char *label, const char *hint, char *buf,
  ...
  const bool focus_requested_by_code =
      (item_status_flags & ImGuiItemStatusFlags_FocusedByCode) != 0;
  ...
  const bool init_make_active =
      (user_clicked || user_scroll_finish || user_nav_input_start ||
  ...
  if ((init_state && g.ActiveId != id) || init_changed_specs) {
    ...
    if (!is_multiline &&
        (focus_requested_by_tabbing || (user_clicked && io.KeyCtrl)))
      select_all = true;
  }
...
bool ImGui::BeginMenu(const char *label, bool enabled) {
  ...
  if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
    ...
    float icon_w =
        0.0f; // FIXME: This not currently exposed for BeginMenu() however you
              // can call window->DC.MenuColumns.DeclColumns(w, 0, 0, 0)
              // yourself
...
bool ImGui::MenuItemEx(const char *label, const char *icon,
  ...
  if (!enabled)
    BeginDisabled(true);
  const ImGuiSelectableFlags flags = ImGuiSelectableFlags_SelectOnRelease |
  ...
  if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
    ...
    pressed = Selectable("", false, flags | ImGuiSelectableFlags_SpanAvailWidth,
                         ImVec2(min_w, 0.0f));
    RenderText(pos + ImVec2(offsets->OffsetLabel, 0.0f), label);
    if (icon_w > 0.0f)
