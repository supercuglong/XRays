...
bool ImGui::BeginTableEx(const char *name, ImGuiID id, int columns_count,
  ...
  if (g.CurrentTableStackIdx + 1 > g.TablesTempDataStack.Size)
    g.TablesTempDataStack.resize(g.CurrentTableStackIdx + 1,
                                 ImGuiTableTempData());
  ...
  if (table->RefScale != 0.0f && table->RefScale != new_ref_scale_unit) {
    ...
    // IMGUI_DEBUG_LOG("[table] %08X RefScaleUnit %.3f -> %.3f, scaling width by
...
void ImGui::EndTable() {
  ...
  const float width_spacings = (table->OuterPaddingX * 2.0f) +
                               (table->CellSpacingX1 + table->CellSpacingX2) *
                                   (table->ColumnsEnabledCount - 1);
  table->ColumnsAutoFitWidth = width_spacings + (table->CellPaddingX * 2.0f) *
                                                    table->ColumnsEnabledCount;
  for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
    if (table->EnabledMaskByIndex & ((ImU64)1 << column_n)) {
      ImGuiTableColumn *column = &table->Columns[column_n];
  ...
  g.CurrentTableStackIdx--;
  temp_data = g.CurrentTableStackIdx >= 0
                  ? &g.TablesTempDataStack[g.CurrentTableStackIdx]
                  : NULL;
  g.CurrentTable =
