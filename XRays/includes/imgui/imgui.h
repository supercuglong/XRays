...
// Tables
// [BETA API] API may evolve slightly! If you use this, please update to the
// next version when it comes out!
...
// Tables: Sorting
// - Call TableGetSortSpecs() to retrieve latest sort specs for the table. NULL
// when not sorting.
// - When 'SpecsDirty == true' you should sort your data. It will be true when
// sorting specs have changed
//   since last call, or the first time. Make sure to set 'SpecsDirty = false'
...
// [BETA API] API may evolve slightly! If you use this, please update to the
// next version when it comes out!
// - Important! Sizing policies have complex and subtle side effects, more so
...
enum ImGuiHoveredFlags_ {
  ...
  ImGuiHoveredFlags_AllowWhenOverlapped =
      1 << 6, // Return true even if the position is obstructed or overlapped by
              // another window
...
struct ImGuiStyle {
  ...
  bool AntiAliasedLinesUseTex; // Enable anti-aliased lines/borders using
                               ...
                               // render with bilinear filtering. Latched at the
                               // beginning of the frame (copied to ImDrawList).
...
struct ImGuiIO {
  ...
  ImVec2 DisplaySize; // <unset>          // Main display size, in pixels
                      // (generally == GetMainViewport()->Size)
  ...
  IMGUI_API void
  AddFocusEvent(bool focused); // Notifies Dear ImGui when hosting platform
                               // windows lose or gain input focus
...
struct ImDrawCmd {
  ...
  unsigned int IdxOffset; // 4    // Start offset in index buffer. Always equal
...
enum ImFontAtlasFlags_ {
  ...
  ImFontAtlasFlags_NoBakedLines =
      1 << 2 // Don't build thick line textures into the atlas (save a little
             // texture memory). The AntiAliasedLinesUseTex features uses them,
             // otherwise they will be rendered using polygons (more expensive
             // for CPU/GPU).
};
...
// static inline ImDrawList* GetOverlayDrawList()            { return
// GetForegroundDrawList(); }                         // OBSOLETED in 1.69 (from
// Mar 2019) static inline void  SetScrollHere(float ratio = 0.5f)     {
// SetScrollHereY(ratio); }                                  // OBSOLETED
...
// (between Oct 2017 and Dec 2017) static inline bool  IsRootWindowFocused() {
// return IsWindowFocused(ImGuiFocusedFlags_RootWindow); }   // OBSOLETED
...
// 0.0f)); }              // OBSOLETED in 1.53 (between Oct 2017 and Dec 2017)
// static inline float GetItemsLineHeightWithSpacing()       { return
// GetFrameHeightWithSpacing(); }                     // OBSOLETED in 1.53
