...
typedef int ImGuiItemFlags;    // -> enum ImGuiItemFlags_          // Flags: for
                               // PushItemFlag()
typedef int ImGuiItemAddFlags; // -> enum ImGuiItemAddFlags_       // Flags: for
                               // ItemAdd()
...
// Flags for ItemAdd()
// FIXME-NAV: _Focusable is _ALMOST_ what you would expect to be called
// '_TabStop' but because SetKeyboardFocusHere() works on items with no TabStop
// we distinguish Focusable from TabStop.
enum ImGuiItemAddFlags_ {
  ImGuiItemAddFlags_None = 0,
  ImGuiItemAddFlags_Focusable =
      1 << 0 // FIXME-NAV: In current/legacy scheme, Focusable+TabStop support
...
enum ImGuiItemStatusFlags_ {
  ...
  ImGuiItemStatusFlags_HasDisplayRect =
      1 << 1, // window->DC.LastItemDisplayRect is valid
  ...
  ImGuiItemStatusFlags_Focused =
      ImGuiItemStatusFlags_FocusedByCode | ImGuiItemStatusFlags_FocusedByTabbing
...
struct ImGuiContext {
  ...
  int TabFocusRequestCurrCounterRegular;  // Any item being requested for focus,
                                          // stored as an index (we on layout to
  // be stable between the frame pressing
  // TAB and the next frame, semi-ouch)
  int TabFocusRequestCurrCounterTabStop; // Tab item being requested for focus,
                                         // stored as an index
  ...
  float ColorEditLastSat; // Backup of last Saturation associated to
                          // LastColor[3], so we can restore Saturation in lossy
                          // RGB<>HSV round trips
...
struct IMGUI_API ImGuiWindowTempData {
  ...
  int FocusCounterRegular; // (Legacy Focus/Tabbing system) Sequential counter,
                           // start at -1 and increase as assigned via
                           // FocusableItemRegister() (FIXME-NAV: Needs
                           // redesign)
  int FocusCounterTabStop; // (Legacy Focus/Tabbing system) Same, but only count
                           // widgets which you can Tab through.
...
//  FocusableItemRegister(...)' (New) IMGUI_VERSION_NUM >= 18209: using
//  'ItemAdd(..., ImGuiItemAddFlags_Focusable)'  and 'bool focused =
//  (GetItemStatusFlags() & ImGuiItemStatusFlags_Focused) != 0'
