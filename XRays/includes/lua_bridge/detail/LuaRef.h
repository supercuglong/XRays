...
public:
  ...
  */
  ...
  /**
      ...
      @param rhs A value to compare with.
      @returns True if the referred value is less than or equal to the specified
     one.
  */
  template <class T> bool operator<=(T rhs) const {
    StackPop p(m_L, 2);
...
class LuaRef : public LuaRefBase<LuaRef, LuaRef> {
  ...
  */
  ...
  public:
    ...
    */
    template <class T> TableItem &operator=(T v) {
      StackPop p(m_L, 1);
    ...
    /**
        ...
        @returns This reference.
    */
    template <class T> TableItem &rawset(T v) {
      StackPop p(m_L, 1);
      lua_rawgeti(m_L, LUA_REGISTRYINDEX, m_tableRef);
      lua_rawgeti(m_L, LUA_REGISTRYINDEX, m_keyRef);
      Stack<T>::push(m_L, v);
    ...
    */
    template <class T> TableItem operator[](T key) const {
