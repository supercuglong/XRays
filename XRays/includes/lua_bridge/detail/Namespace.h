...
/** Provides C++ to Lua registration capabilities.
        ...
        This class is not instantiated directly, call `getGlobalNamespace` to
  ...
  protected:
    ...
    */
    void createConstTable(const char *name, bool trueConst = true) {
      ...
      lua_rawsetp(L, -2,
                  detail::getTypeKey()); // co [typeKey] = name. Stack: ns, co
    ...
    */
    void createClassTable(char const *name) {
      ...
      lua_rawsetp(
          L, -2,
          detail::getPropsetKey()); // cl [propsetKey] = ps. Stack: ns, co, cl

      lua_pushvalue(L, -2); // Stack: ns, co, cl, co
      lua_rawsetp(
          L, -2,
          detail::getConstKey()); // cl [constKey] = co. Stack: ns, co, cl

      lua_pushvalue(L, -1); // Stack: ns, co, cl, cl
      lua_rawsetp(
          L, -3,
          detail::getClassKey()); // co [classKey] = cl. Stack: ns, co, cl
    }
    ...
    */
    void createStaticTable(char const *name) {
      ...
      lua_rawsetp(L, -2, detail::getPropgetKey()); // st [propgetKey] = pg.
                                                   // Stack: ns, co, cl, st

      lua_newtable(L); // Stack: ns, co, cl, st, propset table (ps)
      lua_rawsetp(L, -2, detail::getPropsetKey()); // st [propsetKey] = pg.
                                                   // Stack: ns, co, cl, st

      lua_pushvalue(L, -2); // Stack: ns, co, cl, st, cl
      lua_rawsetp(
          L, -2,
          detail::getClassKey()); // st [classKey] = cl. Stack: ns, co, cl, st
  ...
  public:
    ...
    Class(char const *name, Namespace &parent) : ClassBase(parent) {
      ...
      if (lua_isnil(L, -1)) // Stack: ns, nil
      {
        ...
        lua_rawgetp(L, LUA_REGISTRYINDEX,
                    detail::getConstRegistryKey<T>()); // Stack: ns, st, co
    ...
    */
    Class(char const *name, Namespace &parent, void const *const staticKey)
      ...
      lua_rawgetp(
          L, -1,
          detail::getClassKey()); // Stack: ns, co, cl, st, pst, parent cl (pcl)
      assert(lua_istable(L, -1));

      lua_rawgetp(L, -1, detail::getConstKey()); // Stack: ns, co, cl, st, pst,
                                                 // pcl, parent co (pco)
      assert(lua_istable(L, -1));

      lua_rawsetp(L, -6,
                  detail::getParentKey()); // co [parentKey] = pco. Stack: ns,
                                           // co, cl, st, pst, pcl
      lua_rawsetp(L, -4, detail::getParentKey()); // cl [parentKey] = pcl.
                                                  // Stack: ns, co, cl, st, pst
      lua_rawsetp(L, -2, detail::getParentKey()); // st [parentKey] = pst.
                                                  // Stack: ns, co, cl, st
    ...
    */
    template <class U>
    Class<T> &addStaticProperty(char const *name, U *value,
    ...
    */
    template <class U>
    Class<T> &addStaticData(char const *name, U *value,
                            bool isWritable = true) {
    ...
    /// @param  set        A property setter function pointer, optional,
    /// nullable.
    ...
    */
    ...
    Class<T> &addStaticFunction(char const *name,
                                std::function<ReturnType(Params...)> function) {
      assertStackState(); // Stack: const table (co), class table (cl), static
                          // table (st)
    ...
    */
    template <class MemFn, class C> Class<T> &addConstructor() {
      ...
      lua_pushcclosure(
          L, &ctorContainerProxy<typename detail::FuncTraits<MemFn>::Params, C>,
          0);
    ...
    template <class MemFn> Class<T> &addConstructor() {
      ...
      lua_pushcclosure(
          L, &ctorPlacementProxy<typename detail::FuncTraits<MemFn>::Params, T>,
          0);
      rawsetfield(L, -2, "__call");
  ...
  */
  Namespace(char const *name, Namespace &parent) : Registrar(parent) {
    ...
    if (lua_isnil(L, -1)) // Stack: pns, nil
    {
      ...
      lua_rawsetp(
          L, -2,
          detail::getPropgetKey()); // ns [propgetKey] = pg. Stack: pns, ns

      lua_newtable(L); // Stack: pns, ns, propset table (ps)
      lua_rawsetp(
          L, -2,
          detail::getPropsetKey()); // ns [propsetKey] = ps. Stack: pns, ns
...
public:
  ...
  */
  template <class ReturnType, class... Params>
  Namespace &addFunction(char const *name,
                         std::function<ReturnType(Params...)> function) {
    assert(lua_istable(L, -1)); // Stack: namespace table (ns)
    ...
    lua_pushcfunction(
        L, &CFunc::gcMetaMethodAny<FnType>); // Stack: ns, ud, mt, gc function
