    ...
    while (std::getline(file, line)) {
      ...
      if (delimiterIdx == std::string::npos)
        throw std::runtime_error(
            std::string("Config file line does not contain delimiter `=`: ")
                .append(line));

      rawValues[line.substr(0, delimiterIdx)] =
          line.substr(delimiterIdx + 1, line.length());
    }
...
void ConfigSet::SetPrefixKey(std::string prefix) { prefixKey = prefix; }

std::string ConfigSet::GetPrefixKey() { return prefixKey; }
